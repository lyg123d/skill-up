...
그 서비스가 내부에서 OpenAI 호환 LLM 엔드포인트를 호출합니다.
...
LOCAL_LLM_BACKEND_URL=https://0hinapxjjmzn.shares.zrok.io/v1
LOCAL_TTS_API_URL=https://n3avtssi8lfm.shares.zrok.io/tts
```

프론트는 `LOCAL_MODEL_SERVICE_URL`을 기본으로 바라보지만, 이미지는 `LOCAL_IMAGE_API_URL`이 설정되면 해당 외부 API를 먼저 호출합니다. TTS와 LLM도 각각 지정한 URL을 사용하며, 실패하면 보수적 fallback으로 내려갑니다.

## 로컬 Hugging Face 모델 설정

이미지 생성과 TTS는 OpenAI API를 사용하지 않고, 외부 zrok 엔드포인트 또는 로컬 FastAPI 서비스로 실행합니다.
...
`LOCAL_LLM_BACKEND_URL`는 로컬 모델 서비스가 부르는 OpenAI 호환 LLM 주소입니다. 현재는 제공해주신 zrok 엔드포인트 `https://0hinapxjjmzn.shares.zrok.io/v1`를 기본값으로 사용합니다.
`LOCAL_IMAGE_API_URL`는 이미지 생성을 담당하는 외부 OpenAI 호환 주소입니다. 현재는 제공해주신 zrok 엔드포인트 `https://ngoe5jtsustl.shares.zrok.io`를 기본값으로 사용하고, 내부에서 `/generate`를 호출합니다.
`LOCAL_TTS_API_URL`는 TTS 오디오를 생성하는 외부 OpenAI 호환 주소입니다. 현재는 제공해주신 zrok 엔드포인트 `https://n3avtssi8lfm.shares.zrok.io/tts`를 기본값으로 사용합니다.

FastAPI 서비스는 프로젝트 루트의 `.env.local`과 `.env`를 자동으로 읽습니다.
