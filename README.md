...
- 뉴스 입력부터 브리프, 스크립트, 이미지, TTS, 영상 생성까지 이어지는 전체 제작 플로우를 Next.js 화면에서 한 번에 사용할 수 있도록 연결했습니다.
- FastAPI 로컬 모델 서버와 Next.js API route를 연결해 LLM, 이미지 생성, TTS 요청을 분리하고, 외부 zrok 엔드포인트 또는 로컬 모델로 fallback할 수 있게 구성했습니다.
- 브라우저 `canvas`와 `MediaRecorder`를 사용해 서버 렌더링 없이 9:16 WebM 영상을 생성하도록 구현했습니다.
- TTS는 씬별 음성 파일을 여러 개 합치는 방식에서 전체 내레이션 기준 단일 음성 트랙으로 변경했습니다. 이 방식으로 씬 전환 시 음성이 겹치는 문제를 줄이고, 영상 생성 단계에서 하나의 오디오만 사용하도록 고정했습니다.
- 영상 생성 전에는 단일 TTS 음성이 준비되어야만 렌더링 버튼을 사용할 수 있게 처리했고, 미리듣기 오디오가 재생 중이면 영상 생성 전에 자동으로 정지되도록 했습니다.

### 트러블슈팅

- 문제: 씬이 넘어갈 때 TTS가 2~3개씩 겹쳐 들렸습니다.
- 원인: 기존 구조는 씬별 TTS 세그먼트를 각각 생성하고, 영상 렌더링 시 여러 오디오 소스를 같은 타임라인에 올렸습니다. 이전 브라우저 저장 상태에 남아 있던 `segments` 데이터도 다시 렌더링에 사용될 수 있었습니다.
- 해결: 서버의 씬별 TTS 생성 분기를 제거하고, 프론트에서도 `scenes` 배열을 TTS API로 보내지 않도록 수정했습니다. 렌더러에서는 `segments` 오디오 경로를 제거하고 `audio_url` 하나만 디코딩하도록 변경했습니다.
- 추가 조치: 저장된 예전 씬별 TTS 데이터는 앱 로드 시 무효화하고, 새 음성을 생성하면 기존 영상 결과를 초기화했습니다. 자막 번인은 TTS 안정화가 우선이라 기본 비활성화했습니다.
- 검증: `npm run lint`, `npx tsc --noEmit`, `npm run build`를 실행해 타입, 린트, 프로덕션 빌드를 확인했습니다. 이후 Vercel Production 배포까지 진행했습니다.

### 새롭게 배운 점

- 브라우저에서 영상과 오디오를 합성할 때는 오디오 소스 개수와 시작 시간이 매우 중요하다는 것을 배웠습니다.
- 단순히 재생 시간을 보정하는 것보다, 제품 요구사항에 맞게 오디오 구조를 단순화하는 것이 더 안정적일 수 있었습니다.
- API 응답 구조가 바뀌어도 브라우저 localStorage에 남은 과거 데이터가 계속 문제를 만들 수 있다는 점을 배웠습니다.
- Vercel 서버리스 환경에서는 긴 영상 렌더링을 서버에서 처리하기보다 브라우저 렌더링 또는 별도 worker로 분리하는 구조가 현실적이라는 점을 확인했습니다.
- TTS, 이미지, LLM처럼 외부 모델 API에 의존하는 기능은 실패 fallback과 상태 초기화가 중요하다는 것을 배웠습니다.

### 개선하고 싶은 점

- 현재는 TTS 안정성을 위해 자막 번인을 꺼두었지만, 다음 단계에서는 단일 TTS 길이에 맞춰 자막 타이밍을 자동 분배하는 기능을 추가하고 싶습니다.
- WebM 생성 후 ffmpeg 또는 Remotion 기반 서버 렌더러를 붙여 MP4 변환과 더 안정적인 YouTube 업로드 포맷을 지원하고 싶습니다.
- 영상 생성 결과를 사용자가 다시 편집할 수 있도록 제작 패키지 불러오기 기능을 개선하고 싶습니다.
- TTS 생성 완료 후 오디오 길이를 기준으로 씬 타임라인을 자동 재계산해, 화면 전환과 내레이션 싱크를 더 자연스럽게 맞추고 싶습니다.
- 모델 API 장애가 발생했을 때 사용자에게 어떤 단계가 실패했는지 더 명확히 보여주는 상태 표시와 재시도 UX를 추가하고 싶습니다.
