...
이 프로젝트에서 저는 뉴스 입력부터 브리프, 숏츠 스크립트, 이미지, TTS,
영상 생성, 업로드 준비까지 이어지는 전체 제작 흐름을 하나의 웹 앱으로 연결하는
역할을 맡았습니다. 단순히 화면을 만드는 것보다, 사용자가 뉴스를 넣은 뒤 결과물을
확인하고 다시 수정할 수 있는 작업 흐름을 안정적으로 만드는 데 집중했습니다.

### 담당한 작업

- Next.js 기반 Studio 화면을 구성하고, 뉴스 입력 패널, 후보 카드, 브리프 패널, 스크립트 패널, 씬 타임라인, 영상 생성 패널이 자연스럽게 이어지도록 상태 흐름을 설계했습니다.
- Next.js API route와 FastAPI 로컬 모델 서버를 연결해 LLM, 이미지 생성, TTS 요청을 분리했습니다.
- 외부 zrok 엔드포인트와 로컬 모델을 함께 사용할 수 있도록 환경변수 기반 구조를 정리했습니다.
- 브라우저 `canvas`와 `MediaRecorder`를 사용해 서버 렌더링 없이 9:16 WebM 영상을 생성하도록 구현했습니다.
- Google OAuth와 YouTube resumable upload 구조를 붙일 수 있도록 업로드 API 흐름을 준비했습니다.

### 문제 해결 경험

가장 기억에 남는 문제는 씬이 넘어갈 때 TTS가 2~3개씩 겹쳐 들리던 버그였습니다.
처음에는 단순한 재생 타이밍 문제라고 생각했지만, 확인해보니 씬별 TTS 세그먼트가
여러 개 생성되고 렌더링 단계에서 동시에 올라가는 구조가 원인이었습니다. 여기에
브라우저 localStorage에 남아 있던 이전 `segments` 데이터까지 다시 사용되면서
문제가 반복될 수 있었습니다.

이 문제를 해결하기 위해 TTS 구조를 씬별 여러 음성 파일 방식에서 전체 내레이션
기준 단일 음성 트랙 방식으로 바꿨습니다. 서버에서는 씬별 TTS 생성 분기를 제거하고,
프론트에서는 `scenes` 배열을 TTS API로 보내지 않도록 수정했습니다. 영상 렌더러도
`segments` 오디오 경로를 사용하지 않고 `audio_url` 하나만 디코딩하도록 변경했습니다.

이후 저장된 예전 TTS 데이터는 앱 로드 시 무효화하고, 새 음성을 생성하면 기존 영상
결과를 초기화하도록 처리했습니다. 마지막으로 `npm run lint`, `npx tsc --noEmit`,
`npm run build`를 실행해 타입, 린트, 프로덕션 빌드까지 확인했습니다.

### 배운 점

- AI 기능은 모델 호출 자체보다 입력, 상태, 실패 처리, 재시도 흐름을 안정적으로 설계하는 일이 중요하다는 것을 배웠습니다.
- 브라우저에서 영상과 오디오를 합성할 때는 오디오 소스의 개수와 시작 시간이 결과 품질에 직접적인 영향을 준다는 것을 경험했습니다.
- API 응답 구조를 고쳐도 브라우저에 남은 이전 저장 데이터가 문제를 계속 만들 수 있으므로, localStorage 같은 클라이언트 상태도 마이그레이션 관점에서 관리해야 한다는 점을 배웠습니다.
- Vercel 서버리스 환경에서는 긴 영상 렌더링을 서버에서 처리하기보다 브라우저 렌더링이나 별도 worker로 분리하는 구조가 더 현실적이라는 것을 확인했습니다.

### 앞으로 개선하고 싶은 점

- 단일 TTS 길이에 맞춰 자막 타이밍을 자동 분배하고, 영상에 자연스럽게 번인하는 기능을 추가하고 싶습니다.
- WebM 생성 후 MP4 변환까지 지원해 YouTube 업로드 호환성을 더 높이고 싶습니다.
- 생성된 제작 패키지를 다시 불러와 수정할 수 있는 재편집 기능을 강화하고 싶습니다.
- 모델 API 장애가 발생했을 때 어떤 단계가 실패했는지 더 명확히 보여주는 상태 표시와 재시도 UX를 추가하고 싶습니다.
